...
function loadQuery() {
  ...
  pagesPerSide = urlParams.get('pagesperside') ? urlParams.get('pagesperside') : pagesPerSide;
  showToc = urlParams.get('toc') ? urlParams.get('toc') === 'true' : showToc;
  customCss = urlParams.get('customcss') ? urlParams.get('customcss') : customCss;
}
onMount(async () => {
  mounted = true;
  loadQuery();
...
<div class="container pt-20 mx-auto w-full md:max-w-3xl">
  <div
    class="px-4 w-full text-xl leading-normal text-gray-800 md:px-6"
    style="font-family: Georgia, serif;"
  >
    <div class="font-sans">
      <h1 class="pt-6 pb-2 font-sans font-bold text-gray-900 break-normal text-3xl
		  md:text-4xl">
        Read the web!
      </h1>
    </div>
    <p class="py-6">
      👋 Welcome to the Read the Web! Make the web more readable and printable by generating clean
      pdf.
    </p>
    ...
    <form class="w-full" on:submit="{onSubmit}">
      ...
      <div class="flex flex-wrap -mx-3 mb-2">
        ...
        <FormDropdown label="PAGES PER SIDE" name="pagesperside" bind:value="{pagesPerSide}">
        ...
        <div class="w-full md:w-1/3 px-3 mt-6 mb-6 md:mb-0">
          <details>
            ...
            <label
              class="block mt-3 mb-2 tracking-wide text-gray-700 uppercase text-xs
				font-bold"
              for="customcss"
            >
              CUSTOM CSS
            ...
            <div class="relative">
              <textarea
                ...
                class="block py-3 px-4 pr-8 w-full leading-tight text-gray-700 bg-gray-200 border-gray-200 appearance-none border
				  rounded
